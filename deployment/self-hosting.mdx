...
- **Security**: Requires `seccomp=unconfined` for io_uring support (TigerBeetle client requirement)

<Note>
**Technical Note**: The backend container includes `security_opt: seccomp=unconfined` in the Docker Compose configuration. This is required because the TigerBeetle client library uses io_uring on Linux for high-performance I/O operations. Without this setting, the backend cannot communicate with TigerBeetle. This is already configured in the provided `docker-compose.yml` file.
</Note>
...
- **Security groups**: Allow backend → TigerBeetle, block all external access to TigerBeetle
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="TigerBeetle connection errors">
    ...
    4. Verify backend has `security_opt: seccomp=unconfined` (required for io_uring)
    5. Check backend logs for TigerBeetle client errors: `docker-compose logs backend | grep -i tigerbeetle`
    6. Test connection: `nc -zv tigerbeetle-host 3000`

    **Note**: On Linux, the backend requires `seccomp=unconfined` to use io_uring for TigerBeetle communication. This is pre-configured in the docker-compose.yml file.
  </Accordion>
